...
const designDocs = {
  ...
  content: {
    ...
    views: {
      ...
      user_posts: {
        map: function(doc) {
          if ((doc.type === 'blog' || doc.type === 'forum') && doc.author_id) {
            emit([doc.author_id, doc.type], 1);
          }
          // Also handle legacy documents with embedded author for backward compatibility
          if ((doc.type === 'blog' || doc.type === 'forum') && doc.author && doc.author.id && !doc.author_id) {
            emit([doc.author.id, doc.type], 1);
          }
        }.toString(),
        reduce: function(keys, values) {
          return sum(values);

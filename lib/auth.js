...
// Custom OAuth2Strategy with 3 second delay before token exchange
class DelayOAuth2Strategy extends OAuth2Strategy {
  _oauth2GetOAuthAccessToken(code, params, callback) {
    const performTokenExchange = async () => {
      // Add 3 second delay before token exchange
      logger.info('Adding 3 second delay before token exchange');
    ...
    // Start the token exchange process
    performTokenExchange().catch(err => {
      logger.error('Unexpected error in token exchange', { error: err.message });
      callback(err);
    });
  }
}

passport.use('zombieauth', new DelayOAuth2Strategy({

...
class RetryOAuth2Strategy extends OAuth2Strategy {
  _oauth2GetOAuthAccessToken(code, params, callback) {
    ...
    const attemptTokenExchange = async (attempt) => {
    ...
    const retryWithBackoff = async () => {
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          const result = await attemptTokenExchange(attempt);
          return callback(null, result.accessToken, result.refreshToken, result.results);
        } catch (error) {
    ...
    retryWithBackoff().catch(err => {
      logger.error('Unexpected error in retry logic', { error: err.message });
      callback(err);
    });
  }
}

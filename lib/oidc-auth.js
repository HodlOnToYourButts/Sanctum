...
async function handleCallback(req, res) {
  try {
    ...
    try {
      // Try to get existing user document to preserve _rev
      const existingUser = await db.get(userInfo.sub);
      userDoc._rev = existingUser._rev;
    } catch (error) {
      // User document doesn't exist, will create new one
    }

    await db.insert(userDoc);

    logger.info('User document updated in database', {
      userId: userInfo.sub,
      email: userInfo.email,
      name: userInfo.name
    });
    ...
    req.session.oidc_user = sessionUser;
